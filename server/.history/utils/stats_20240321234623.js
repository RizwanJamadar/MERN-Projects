...
const calculateLeaveStats = (leaveData) => {
  let sickLeave = { count: 0, days: 0 };
  let annualLeave = { count: 0, days: 0 };
  let casualLeave = { count: 0, days: 0 };
  let otherLeave = { count: 0, days: 0 };
    ...
    switch (leave.type) {
      case 'Sick Leave':
        if (leave.status === 'Approved') {
          sickLeave.count++;
          sickLeave.days += leave.days;
        }
        break;
      case 'Annual Leave':
        if (leave.status === 'Approved') {
          annualLeave.count++;
          annualLeave.days += leave.days;
        }
        break;
      case 'Casual Leave':
        if (leave.status === 'Approved') {
          casualLeave.count++;
          casualLeave.days += leave.days;
        }
        break;
      default:
        if (leave.status === 'Approved') {
          otherLeave.count++;
          otherLeave.days += leave.days;
        }
    }
  });
