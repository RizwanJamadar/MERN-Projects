...
export const getDeptStaff = async(req,res,next) =>{
  const user = req.user;
  try {
    // // const hod = await User.findOne({ department: user.department, Role: "hod" });
    // console.log(user);

    const hod = await User.findOne(user.id) ;
    console.log(hod);
    ...
    const professors = await User.find({ department: user.department, Role: "Professor" });
