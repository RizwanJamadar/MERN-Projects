...
export const approveRequest = async (req, res, next) => {
  ...
  try {
    ...
    if (leaveRequest.hodApproval === null || leaveRequest.vpApproval === null) {
      ...
      (leaveRequest.hodApproval == true && leaveRequest.vpApproval == true) ||
      (leaveRequest.hodApproval == false && leaveRequest.vpApproval == true)
    ) {
...
export const getDeptStaff = async (req, res, next) => {
  ...
  try {
    ...
    if (!hod) {
      return res
        .status(404)
        .json({ message: "HOD not found for this department" });
    }
    ...
    if (!professors || professors.length === 0) {
      return res
        .status(404)
        .json({ message: "No professors found in this department" });
    }

    // Fetch leave requests of professors in the HOD's department
    const leaveRequests = await LeaveRequest.find({
      userId: { $in: professors.map((prof) => prof._id) },
...
export const getCountByVp = async (req, res, next) => {
  try {
    var todayFormatted = new Date().toISOString().split("T")[0];
    const staffCount = await UserModel.countDocuments();
    const staffOnLeave = await LeaveRequestModel.countDocuments({
      startDate: todayFormatted,
    });
    const workingStaff = staffCount - staffOnLeave;
    res.status(200).json({
      staffCount,
      staffOnLeave,
      workingStaff,
...
export const getCountByHod = async (req, res, next) => {
  try {
