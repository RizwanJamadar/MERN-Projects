...
const AllLeaves = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(res.data);
        setLeaves(res.data.leaves); // Set all leaves from API
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const filteredLeaves = category ? leaves.filter((leave) => leave.status === category) : leaves;
  ...
  return (
    <>
      <div className="leave">
        <PaginationTable columns={columns} Data={filteredLeaves} count={5} />
      </div>
    </>
  );
};
