...
const BarChartGraph = () => {
  ...
  const getUserData = async () => {
    try {
      ...
      const leavesData = res.data; // Assuming res.data.leaves is an array
      // setLeaves(leavesData); // Set the leaves state with the received data
  
      const stats = await axios.post("http://localhost:8800/calculateByMonth", { leaves: leavesData }); // Send the array as part of the request body
