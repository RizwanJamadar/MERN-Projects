...
const calculateByMonth = async (req, res, next) => {
  try {
    if (!Array.isArray(array.leaves)) {
      console.log("Invalid 'leaves' data. Expected an array.");
    }

    const leaveData = array.leaves;
    const leaveStats = calculateLeaveStatsByMonth(leaveData);
    res.status(200).send(leaveStats);
  } catch (err) {
    console.error("Error calculating leave stats:", err);
    next(err);
